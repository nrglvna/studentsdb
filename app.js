...
function loadData() {
  ...
  fetch(getURL)
    ...
    .then(n => {
      document.getElementById("status").innerHTML = n + " жазба жүктелді"
      if (n > 0) {
        selectRow()
        scrollToSelection()
      }
...
function addToTable(doc) {
  ...
  cell2.innerHTML = doc.speciality
  cell3.innerHTML = doc.age
  cell4.innerHTML = "<input type='radio' name='select' onclick='selectRow(this)' checked>"
  cell4.className = "tradio"
...
function setSelection(row) {
  ...
  document.getElementById("speciality").value = row.cells.item(1).innerHTML
  document.getElementById("age").value = row.cells.item(2).innerHTML
...
function addData() {
  ...
  postToDB({ name: name, speciality: speciality, age:age})
}
